...
const navigation = [
  ["New in", "/shop?tag=New+In"],
  ["Apparel", "/shop?category=Apparel"],
  ["Bags & accessories", "/shop?category=Bags+%26+Accessories"],
  ["Beauty", "/shop?category=Beauty"],
  ["Fragrances", "/shop?category=Fragrance"],
  ["Modest edit", "/shop?tag=Modest+Edit"],
  ["Gifts", "/shop?tag=Gifts"],
] as const;
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {cart?.items.length ? cart.items.map(item => (
            <article key={item.lineId} className="flex gap-4 border-b border-[#e5ded2] py-4 first:pt-0">
              <img className="h-24 w-20 object-cover" src={item.image?.url ?? "/manus-storage/alraheem-accessories-flatlay_814b21f8.jpg"} alt={item.image?.altText ?? item.productTitle} />
...
function Header() {
  ...
  return <header className="bg-[#f9f6ef]"><div className="announcement-bar"><p><PackageCheck size={12} /> A considered edit for every day — complimentary delivery on orders over PKR 5,000</p><Link className="announcement-link" href="/track-order">Track your order <ArrowRight size={12} /></Link></div><div className="utility-nav"><div className="utility-links"><Link href="/shop?category=Beauty">Beauty</Link><Link href="/shop?category=Apparel">Fashion</Link><Link href="/shop?tag=New+In">The 786 edit</Link></div><Link href="/track-order">Track your order</Link></div><div className="container header-main"><Link href="/" className="brand-lockup"><img className="brand-logo" src={LOGO} alt="ALRAHEEM COLLECTION 786" /><span className="brand-name">ALRAHEEM<em>COLLECTION 786</em></span></Link><form className="search-field" onSubmit={submitSearch}><Search size={17} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search the collection" aria-label="Search the collection" /><button type="submit" aria-label="Search"><ArrowRight size={15} /></button></form><div className="header-actions"><Link href="/contact" aria-label="Contact ALRAHEEM COLLECTION 786"><UserRound size={20} /></Link><Link href="/wishlist" aria-label="View saved items" className="relative"><Heart size={20} />{count > 0 && <span className="count-badge">{count}</span>}</Link><button type="button" aria-label="Open shopping bag" onClick={openCart}><ShoppingBag size={20} />{itemCount > 0 && <span className="count-badge">{itemCount}</span>}</button><button className="mobile-menu-button" type="button" onClick={() => setOpen(value => !value)} aria-label="Toggle navigation">{open ? <X size={22} /> : <Menu size={22} />}</button></div></div><nav className="main-nav" aria-label="Primary navigation">{navigation.map(([label, href]) => <Link key={label} href={href}>{label}</Link>)}</nav>{open && <div className="border-t border-[#d8d0c2] bg-[#fffdf9] px-5 py-4 md:hidden"><div className="grid gap-3">{navigation.map(([label, href]) => <Link key={label} onClick={() => setOpen(false)} href={href} className="border-b border-[#eee8dd] pb-3 text-[11px] font-extrabold uppercase tracking-[.12em] text-[#123f72]">{label}</Link>)}<Link href="/track-order" onClick={() => setOpen(false)} className="pt-1 text-[11px] font-extrabold uppercase tracking-[.12em] text-[#bb492d]">Track your order</Link></div></div>}</header>;
}

function Footer() {
  return <footer className="bg-[#123f72] text-[#f9f6ef]"><div className="container grid gap-10 py-12 md:grid-cols-[1.5fr_repeat(3,1fr)]"><div><div className="brand-lockup text-[#f9f6ef]"><img className="brand-logo" src={LOGO} alt="" /><span className="brand-name">ALRAHEEM<em className="text-[#f3b49d]">COLLECTION 786</em></span></div><p className="mt-5 max-w-60 text-xs leading-6 text-[#cbd6e2]">Everyday, arranged with intention. A considered edit of fashion, fragrance and gifting.</p></div><div><p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-[#f3b49d]">Shop</p><div className="mt-4 grid gap-2 text-xs text-[#dce5ee]"><Link href="/shop">All pieces</Link><Link href="/shop?category=Apparel">Apparel</Link><Link href="/shop?category=Bags+%26+Accessories">Bags & accessories</Link></div></div><div><p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-[#f3b49d]">Help</p><div className="mt-4 grid gap-2 text-xs text-[#dce5ee]"><Link href="/bag">Your bag</Link><Link href="/track-order">Delivery details</Link><Link href="/contact">Contact us</Link></div></div><div><p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-[#f3b49d]">Stay in the loop</p><p className="mt-4 text-xs leading-6 text-[#dce5ee]">Follow the collection for new arrivals and private edits.</p><Link href="/contact" className="mt-4 inline-flex items-center gap-2 text-[10px] font-extrabold uppercase tracking-[.14em] text-[#f9f6ef] underline underline-offset-4">Get in touch <ArrowRight size={13} /></Link></div></div><div className="border-t border-white/15 py-4 text-center text-[9px] uppercase tracking-[.14em] text-[#a9bacd]">© 2026 ALRAHEEM COLLECTION 786 · Curated with intention</div></footer>;
}
