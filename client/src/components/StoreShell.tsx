        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {cart?.items.length ? cart.items.map(item => (
            <article key={item.lineId} className="flex gap-4 border-b border-[#e5ded2] py-4 first:pt-0">
              <img className="h-24 w-20 object-cover" src={item.image?.url ?? STORE_CATEGORIES[0].image} alt={item.image?.altText ?? item.productTitle} />
